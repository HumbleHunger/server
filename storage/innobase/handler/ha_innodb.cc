...
MY_ALIGNED(CPU_LEVEL1_DCACHE_LINESIZE) static log_flush_requests log_requests;
...
void log_flush_notify(lsn_t flush_lsn)
{
  if (log_requests.start)
  {
    ...
    {
      log_flush_request *entry= pending, *last_ready= pending;
      ...
      But there is no harm in delaying notifications for those a
      bit. And in practise, the list is unlikely to have more than one
      element anyway, because the redo log would be flushed every
      srv_flush_log_at_timeout seconds (1 by default). */
      for (; entry && entry->lsn <= flush_lsn;
           last_ready= entry, entry= entry->next);

      if (last_ready)
      {
        /* Detach the head of the list that corresponds to log writes
        that are now persistent. */
...
static void innodb_log_flush_request(void *cookie)
{
  const lsn_t lsn= log_sys.get_lsn(), flush_lsn= log_sys.get_flushed_lsn();
...
completed:
  ...
  srv_sync_log_buffer_in_background(), which should initiate a
  log flush about once every srv_flush_log_at_timeout seconds.
  But, starting with the innodb_force_recovery=2 level, that
  background task will not run. */
  if (UNIV_UNLIKELY(srv_force_recovery >= SRV_FORCE_NO_BACKGROUND))
  {
    log_write_up_to(lsn, true);
    goto completed;
  }

  log_flush_request *req= static_cast<log_flush_request*>
  ...
  mysql_mutex_lock(&log_requests.mutex);
  if (log_requests.end)
    /* Append the entry to the list. Because we invoked
    log_sys.get_lsn() before acquiring the mutex, this list may not be
    in ascending order of req->lsn. */
